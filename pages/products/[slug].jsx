...
const PRODUCT_REVIEWS = {
  ...
  'hgh-191aa': [
    ...
    { name: 'K. Osei', date: 'Mar 10, 2025', rating: 5, title: 'Great service, fast shipping', body: 'Placed order Wednesday, received Friday in Tampa-area packaging. Intact vial, lyophilized cake perfect. Customer support answered a technical question within 2 hours.', verified: true },
  ],
  'nad': [
...
const CATEGORY_REVIEWS = {
  'Healing & Regeneration': [
    ...
    { name: 'T. Collins', date: 'Feb 19, 2025', rating: 4, title: 'Very good quality', body: 'Tested to 98.7% purity independently. Happy with the result. Fast delivery from Florida.', verified: true },
  ],
...
export default function ProductDetail({ product, related, stackProducts, coa, catalogMode = 'gated', isGuest: serverIsGuest = false }) {
  ...
  // Products available for purchase — everything else is "Coming Soon"
  const AVAILABLE_CATEGORIES = ['GLP-1 Research Peptides'];
  const AVAILABLE_SLUGS = ['ghk-cu'];
  const comingSoon = !AVAILABLE_CATEGORIES.includes(product.category) && !AVAILABLE_SLUGS.includes(product.slug);
    ...
    try {
      if (typeof window !== 'undefined' && window.omnisend) {
        ...
        window.omnisend.push(['track', '$productViewed', {
          ...
          $currency: 'USD',
          $price: itemPrice,
          $imageUrl: product.image ? `https://evolabsresearch.co${product.image}` : '',
          $productUrl: `https://evolabsresearch.co/products/${product.slug}`,
          $title: product.name,
        }]);
      }
    } catch (_) {}
      ...
      fetch('/api/omnisend/track', {
        ...
        body: JSON.stringify({
          ...
          fields: {
            ...
            productUrl: typeof window !== 'undefined' ? window.location.href : `https://evolabsresearch.co/products/${product.slug}`,
  ...
  const breadcrumbSchema = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://evolabsresearch.co' },
      { '@type': 'ListItem', position: 2, name: 'Products', item: 'https://evolabsresearch.co/products' },
      { '@type': 'ListItem', position: 3, name: product.name, item: `https://evolabsresearch.co/products/${product.slug}` },
    ],
  };
  ...
  const productSchema = {
    ...
    offers: {
      '@type': 'Offer',
      url: `https://evolabsresearch.co/products/${product.slug}`,
      priceCurrency: 'USD',
            ...
            <div style={{ paddingTop: 8 }}>
              ...
              {isOOS ? (
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  {comingSoon && (
                    <div style={{ background: 'linear-gradient(135deg, #06b6d4, #0891b2)', color: '#fff', borderRadius: 12, padding: '16px 24px', textAlign: 'center', marginBottom: 10 }}>
                      <span style={{ fontSize: 15, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", letterSpacing: '0.04em' }}>Coming Soon to Canada</span>
                      <p style={{ fontSize: 12, opacity: 0.85, margin: '4px 0 0', fontFamily: "'DM Sans', sans-serif" }}>This product will be available shortly. Check back soon!</p>
                    </div>
                  )}
                  <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                    <button
                      ...
                      style={{
                        ...
                        background: comingSoon ? '#9ca3af' : cartAdded ? '#16a34a' : '#000000',
                        transition: 'background 0.3s',
                        cursor: comingSoon ? 'not-allowed' : 'pointer',
                        opacity: comingSoon ? 0.6 : 1,
                      }}
                    >
                      {comingSoon ? 'Coming Soon' : cartAdded ? '✓ Added to Cart' : 'Add to Cart →'}
                    </button>
                    {coa && (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 16 }}>
                {[
                  ...
                  { icon: '🇺🇸', label: 'USA Shipped', sub: 'Tampa, FL' },
                ].map(b => (
            ...
            {(() => {
              ...
              if (activeTab === 'shipping') return (
                <div style={{ maxWidth: 660 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    {[
                      { title: 'Shipping', body: 'Orders are shipped from our Tampa, FL fulfillment center. Standard shipping typically delivers in 3–5 business days. Expedited 2-day shipping available at checkout. Orders over $250 ship free.' },
                ...
                <div style={{ maxWidth: 660 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    {[
                      ...
                      { q: 'How is this peptide shipped?', a: 'Lyophilized (freeze-dried) in a sealed vial. Ships from Tampa, FL with temperature-appropriate packaging. Standard 3–5 day delivery or 2-day expedited available.' },
              ...
              <div className="container" style={{ padding: '18px 24px 20px' }}>
                ...
                <div className="sticky-bar-row" style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                  ...
                  <button
                    ...
                    style={{
                      flexShrink: 0,
                      background: comingSoon ? '#9ca3af' : cartAdded ? '#16a34a' : '#1B4D3E',
                      color: '#fff',
                      border: 'none',
                      borderRadius: 12,
                      padding: '14px 32px',
                  ...
                  >
                    {comingSoon ? 'Coming Soon' : cartAdded ? '✓ Added' : 'Add to Cart'}
                  </button>
                </div>
              </div>
            </div>
