...
function isProductAvailable(product) {
  return AVAILABLE_CATEGORIES.includes(product.category) || AVAILABLE_SLUGS.includes(product.slug);
}

function ProductCard({ product }) {
  const isOOS = product.outOfStock;
  const comingSoon = !isProductAvailable(product);
  const { toggle, isWishlisted } = useWishlist();
  const wishlisted = isWishlisted(product.slug);
  const { addToCompare, isInCompare } = useCompare();
  const inCompare = isInCompare(product.slug);
  return (
    <Link href={comingSoon ? '#' : `/products/${product.slug}`} onClick={comingSoon ? e => e.preventDefault() : undefined} style={{ display: 'flex', flexDirection: 'column', textDecoration: 'none', background: '#fff', borderRadius: 20, border: '1.5px solid #e5e7eb', overflow: 'hidden', transition: 'transform 0.25s ease, box-shadow 0.25s ease', opacity: comingSoon ? 0.55 : 1, filter: comingSoon ? 'grayscale(60%)' : 'none', cursor: comingSoon ? 'default' : 'pointer' }} onMouseEnter={e => { if (!comingSoon) { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 12px 40px rgba(0,0,0,0.10)'; } }} onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = 'none'; }}>
      <div style={{
        background: '#ffffff',
        ...
        {comingSoon && (
          <span style={{
            ...
            fontSize: 9, fontWeight: 700, padding: '4px 10px', borderRadius: 9999,
            letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: "'DM Sans', sans-serif",
            boxShadow: '0 2px 8px rgba(6,182,212,0.3)',
          }}>Coming Soon</span>
        )}
        {!comingSoon && product.badge && (
          <span style={{
      ...
      <div style={{ padding: '14px 16px 18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, marginTop: 'auto' }}>
          <div>
            <span style={{ fontSize: 15, fontWeight: 700, color: comingSoon ? '#9ca3af' : '#131315', fontFamily: "'Poppins', sans-serif" }}>
              {comingSoon ? '' : isOOS ? 'Sold Out' : product.salePrice || product.price}
            </span>
            {!comingSoon && product.salePrice && !isOOS && (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {comingSoon ? (
              <span style={{ background: '#e5e7eb', color: '#6b7280', fontSize: 11, fontWeight: 600, padding: '7px 14px', borderRadius: 9999, fontFamily: "'DM Sans', sans-serif" }}>
                Coming Soon
              </span>
            ) : !isOOS && (
...
export default function ProductsPage({ products, catalogMode = 'gated', isGuest = false }) {
  ...
  return (
    <Layout title="All Research Peptides | EVO Labs Research" description="Browse all 48+ research-grade peptides from EVO Labs. 99%+ purity guaranteed. Filter by category, sort by price. COA available for every batch.">
      {/* Page Header */}
